...
| PG_POOL_MAX_COUNT       | Maximum connections in the shared pool (default `10`). The pool is process-wide and stays open; size it so _dynos × PG_POOL_MAX_COUNT_ stays below the database connection limit. |
| PG_IDLE_TIMEOUT_SECONDS | Seconds an idle connection is kept before it is released (default `30`). Lower frees connections sooner, higher keeps them warm.                                                  |
| PGHOST                  |
| PGUSER                  |
| PGDATABASE              |
| ENDPOINT_ID             |
