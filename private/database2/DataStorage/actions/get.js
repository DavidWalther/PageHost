  ...
  static createStrategy(leftTable, rightTable, orderDirection, rightSortOrder) {
    ...
    if (!rightTable && orderDirection) {
      Logging.debugMessage({
        severity: 'FINEST',
        location: 'OrderStrategyFactory.createStrategy',
        message: `Creating LeftOrderWithoutTablePrefix`,
      });
      strategy = new LeftOrderWithoutTablePrefix().setOrderDirection(
        orderDirection
      );
    }
    if (rightTable && orderDirection && !rightSortOrder) {
      Logging.debugMessage({
        severity: 'FINEST',
        location: 'OrderStrategyFactory.createStrategy',
        message: `Creating LeftOrderWithTablePrefix`,
      });
      strategy = new LeftOrderWithTablePrefix()
        .setLeftTable(leftTable)
        .setOrderDirection(orderDirection);
    }
    if (rightTable && !orderDirection && rightSortOrder) {
      Logging.debugMessage({
        severity: 'FINEST',
        location: 'OrderStrategyFactory.createStrategy',
        message: `Creating RightOrderWithTablePrefix`,
      });
      strategy = new RightOrderWithTablePrefix()
        .setRightTable(rightTable)
        .setRightSortOrder(rightSortOrder);
    }
    if (rightTable && orderDirection && rightSortOrder) {
      Logging.debugMessage({
        severity: 'FINEST',
        location: 'OrderStrategyFactory.createStrategy',
        message: `Creating CombinedLeftRightOrder`,
      });
      strategy = new CombinedLeftRightOrder()
        .setLeftTable(leftTable)
        .setRightTable(rightTable)
        .setOrderDirection(orderDirection)
        .setRightSortOrder(rightSortOrder);
...
class PublishDateUndefined extends PublishDateCondition {
  getCondition() {
    ...
    if (this.rightTable) {
      condition += this.tableName
        ? `${this.tableName}.PublishDate <= NOW()`
        : `PublishDate <= NOW()`;
...
class PublishDateActual extends PublishDateCondition {
  getCondition() {
    ...
    if (this.rightTable) {
      condition += this.tableName
        ? `${this.tableName}.PublishDate <= '${datePart} ${timeWithoutMs}'`
        : `PublishDate <= '${datePart} ${timeWithoutMs}'`;
...
class ActionGet {
  ...
  getLeftTableApplicationKey() {
    ...
    applicationIncludedCriteria.push(
      `${tablePrefix}applicationIncluded LIKE '%' || '${this.conditionApplicationKey}' || '%'`
    );
    applicationIncludedCriteria.push(`${tablePrefix}applicationIncluded = '*'`);

    let applicationExcludedCriteria = [];
    applicationExcludedCriteria.push(
      `${tablePrefix}applicationExcluded isNull`
    );
    applicationExcludedCriteria.push(
      `${tablePrefix}applicationExcluded NOT LIKE '%' || '${this.conditionApplicationKey}' || '%'`
    );
  ...
  getRightTableApplicationKey() {
    ...
    rightApplicationIncludedCriteria.push(
      `${rightTableName}.applicationIncluded LIKE '%' || '${this.conditionApplicationKey}' || '%'`
    );
    rightApplicationIncludedCriteria.push(
      `${rightTableName}.applicationIncluded = '*'`
    );

    let rightApplicationExcludedCriteria = [];
    rightApplicationExcludedCriteria.push(
      `${rightTableName}.applicationExcluded isNull`
    );
    rightApplicationExcludedCriteria.push(
      `${rightTableName}.applicationExcluded NOT LIKE '%' || '${this.conditionApplicationKey}' || '%'`
    );
  ...
  setLeftJoin(rightTableClass, joinCondition) {
    if (!rightTableClass || !joinCondition) {
      return this;
    }
  ...
  createCombinedCriteria() {
    let conditionArray = [];
    if (this.conditionRecordId) {
      conditionArray.push(this.conditionRecordId);
    }

    // Add left table publish date condition if it was set (even if undefined)
    if (
      this.publishDateTime !== null &&
      this.hasOwnProperty('publishDateTime')
    ) {
      let leftTableName = this.leftTable?.getTableName()() || this.tableName;
      ...
      if (leftPublishCondition) {
        if (this.rightTable) {
          ...
          if (leftPublishCondition.includes('PublishDate <= NOW()')) {
            ...
            leftPublishCondition = leftPublishCondition.replace(
              'PublishDate <=',
              `${leftTableName}.PublishDate <=`
            );
          }
        }
    ...
    if (this.conditionApplicationKey) {
      conditionArray.push(this.getConditionApplicationKey());
    }
  ...
  getFieldString() {
    if (this.rightTable) {
      ...
      let rightTableFieldNames = rightTableFields.map(
        (field) =>
          `${rightTableName}.${field} as ${rightTableFieldPrefix}_${field}`
      );

      let leftTableName = this.leftTable?.getTableName()() || this.tableName;
      let leftTableFieldPrefix = leftTableName.toLowerCase();
      let leftTableFields =
        this.leftTable?.getTableFields()() || this.tableFields;
      let leftTableFieldNames = leftTableFields.map(
        (field) =>
          `${leftTableName}.${field} as ${leftTableFieldPrefix}_${field}`
      );
  ...
  getOrderString() {
    const sortmode = OrderStrategyFactory.createStrategy(
      this.leftTable,
      this.rightTable,
      this.orderDirection,
      this.rightSortOrder
