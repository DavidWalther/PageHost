...
class UnpublishEndpoint extends EndpointLogic {
  ...
  async execute() {
    ...
    const environmentAllowedDmls =
      this.environment.APPLICATION_ACTIVE_ACTIONS || '[]';
    let allowedDmls = JSON.parse(environmentAllowedDmls).map((permission) =>
      permission.toLowerCase()
    );
    Logging.debugMessage({
      severity: 'INFO',
      message: `Parsed allowed DMLs: ${JSON.stringify(allowedDmls)}`,
      location: LOCATION,
    });
    ...
    try {
      ...
      if (!this.isAllowedPublish) {
        Logging.debugMessage({
          severity: 'INFO',
          message: 'Permission denied for publish operation',
          location: LOCATION,
        });
        this.responseObject
          .status(403)
      ...
      if (!existingRecord.publishdate) {
        ...
        this.responseObject
          .status(400)
          .json({ success: false, error: 'Record is already unpublished' });
        return;
      }
  ...
  get isAllowedPublish() {
    ...
    Logging.debugMessage({
      severity: 'FINEST',
      message: `Checking if publish is allowed`,
      location: LOCATION,
    });
  ...
  validateInput() {
    ...
    if (!object || typeof object !== 'string') {
      Logging.debugMessage({
        severity: 'INFO',
        message: 'Missing or invalid object parameter',
        location: LOCATION,
      });
      return false;
    }
    ...
    if (!id || typeof id !== 'string') {
      Logging.debugMessage({
        severity: 'INFO',
        message: 'Missing or invalid id in payload',
        location: LOCATION,
      });
      return false;
    }
    ...
    if (!supportedObjects.includes(object.toLowerCase())) {
      Logging.debugMessage({
        severity: 'INFO',
        message: `Unsupported object type: ${object}`,
        location: LOCATION,
      });
      return false;
    }
  ...
  async getRecord(object, id) {
    ...
    try {
      ...
      Logging.debugMessage({
        severity: 'ERROR',
        message: `Failed to get record: ${error.message}`,
        location: LOCATION,
      });
      throw error;
    }
  ...
  async unpublishRecord(object, id) {
    ...
    try {
      ...
      Logging.debugMessage({
        severity: 'INFO',
        message: `Record unpublished successfully: ${object} ${id}`,
        location: LOCATION,
      });
      ...
      Logging.debugMessage({
        severity: 'ERROR',
        message: `Failed to unpublish record: ${error.message}`,
        location: LOCATION,
      });
      throw error;
    }
