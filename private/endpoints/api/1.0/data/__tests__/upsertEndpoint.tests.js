...
describe('UpsertEndpoint', () => {
  ...
  it('should perform update when id is present and edit is allowed', async () => {
    ...
    const mockUpdateData = jest
      .fn()
      .mockResolvedValue({ id: 'existingid', key: 'value' });
    DataFacade.mockImplementation(() => ({
      setSkipCache: () => ({ updateData: mockUpdateData }),
    }));
    endpoint = new UpsertEndpoint()
      .setEnvironment(environment)
      .setRequestObject(req)
    ...
    expect(res.json).toHaveBeenCalledWith({
      success: true,
      result: { id: 'existingid', key: 'value' },
    });
