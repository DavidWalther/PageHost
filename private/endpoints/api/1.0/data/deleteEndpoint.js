...
class DeleteEndpoint extends EndpointLogic {
  async execute() {
    ...
    if (!object || !id) {
      Logging.debugMessage({
        severity: 'INFO',
        message: 'Missing object or id in query',
        location: LOCATION,
      });
      this.responseObject
        .status(400)
        .json({ success: false, error: 'Missing object or id' });
      return;
    }
    ...
    if (!this.isAllowedDelete) {
      Logging.debugMessage({
        severity: 'INFO',
        message: 'Permission denied for delete operation',
        location: LOCATION,
      });
      this.responseObject
        .status(403)
  ...
  get isAllowedDelete() {
    ...
    Logging.debugMessage({
      severity: 'FINEST',
      message: `Checking if delete is allowed`,
      location: LOCATION,
    });
