...
class UpsertEndpoint extends EndpointLogic {
  async execute() {
    ...
    const environentAllowedDmls =
      this.environment.APPLICATION_ACTIVE_ACTIONS || '[]';
    let allowedDmls = JSON.parse(environentAllowedDmls).map((permission) =>
    ...
    Logging.debugMessage({
      severity: 'INFO',
      message: `DML Use Case: ${dmlUseCase}`,
      location: LOCATION,
    });
    ...
    switch (dmlUseCase) {
      case 'create': {
        if (!this.isAllowedCreate) {
          Logging.debugMessage({
            severity: 'INFO',
            message: 'Permission denied for create operation',
            location: LOCATION,
          });
          this.responseObject
            .status(403)
      ...
      case 'update': {
        if (!this.isAllowedEdit) {
          Logging.debugMessage({
            severity: 'INFO',
            message: 'Permission denied for update operation',
            location: LOCATION,
          });
          this.responseObject
            .status(403)
      ...
      default: {
        Logging.debugMessage({
          severity: 'INFO',
          message: 'Invalid DML use case',
          location: LOCATION,
        });
        this.responseObject
          .status(400)
          .json({ success: false, error: 'Invalid DML use case' });
        return;
      }
    }
  }
  ...
  async executeUpdate() {
    ...
    try {
      Logging.debugMessage({
        severity: 'INFO',
        message: 'Upsert operation started',
        location: LOCATION,
      });
      ...
      Logging.debugMessage({
        severity: 'ERROR',
        message: `Upsert operation failed: ${error.message}`,
        location: LOCATION,
      });
      this.responseObject
        .status(500)
  ...
  get isAllowedEdit() {
    ...
    Logging.debugMessage({
      severity: 'FINEST',
      message: `Checking if edit is allowed`,
      location: LOCATION,
    });
  ...
  get isAllowedCreate() {
    ...
    Logging.debugMessage({
      severity: 'FINEST',
      message: `Checking if create is allowed`,
      location: LOCATION,
    });
