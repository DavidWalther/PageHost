...
app.get('/data/query/*', async (req, res) => {
  ...
  if (bearerToken) {
    ...
    let userScopes = await accessTokenService
      .setEnvironment(environment)
      .getScopesForBearer(bearerToken);
...
app.post('/api/1.0/new-endpoint', async (req, res) => {
  ...
  if (
    !accessTokenService.isBearerValidFromScope(bearerToken, ['required_scope'])
  ) {
    res.status(401).send('Unauthorized');
    return;
  }
